Sub AddNewColumns()
    Dim ws As Worksheet
    Set ws = ThisWorkbook.Sheets("KPI Candidate Start Date 2024 w")
...
Sub ApplyTextToColumns()
    ...
    Set ws = ThisWorkbook.Sheets("KPI Candidate Start Date 2024 w")
     ...
     With ws
        ...
        .Columns("F").Replace What:="??-", Replacement:="", LookAt:=xlPart, SearchOrder:=xlByRows, MatchCase:=False, SearchFormat:=False, ReplaceFormat:=False

        .Range("AF2").Formula = "=IFNA(VLOOKUP(AE2, [CC_MAP.xlsx]Sheet1!$A:$G,7,0),IF(LEFT(H2,1)=""A"",""Atos"",""Eviden""))"
        .Range("AG2").Formula = "=IFNA(VLOOKUP(AE2, [CC_MAP.xlsx]Sheet1!$A:$E,5,0),XLOOKUP(ah2,[CC_MAP.xlsx]Sheet1!$F:$F,[CC_MAP.xlsx]Sheet1!$E:$E))"
        .Range("AH2").Formula = "=IFNA(VLOOKUP(AE2, [CC_MAP.xlsx]Sheet1!$A:$F,6,0),F2)"
...
Sub CreatePivotTable()
    ...
    Set DSheet = Worksheets("KPI Candidate Start Date 2024 w")
